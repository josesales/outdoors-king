    ...
    return (
        <Link className='flex flex-none' to='/checkout'>
            <div className="relative">
                ...
                <img title={productsNumber && productsNumber > 0 ? `${productsNumber} Products in your Cart.` : 'Your Cart is Empty.'}
                    src={cart} alt="Your Cart" className="w-10 h-6 sm:w-20 sm:h-16" />
                {
                    productsNumber && productsNumber > 0 ?
                        <span title={`${productsNumber} Products in your Cart.`}
                            ...
                            text-sm sm:${globalStyles.textDefault}`}>{productsNumber}</span>
                        : null
