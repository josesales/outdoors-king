const globalStyles = {
  ...
  input: `flex-initial px-4 w-10/12 sm:w-9/12 md:w-6/12 lg:w-1/3 h-16 rounded-2xl mb-12 focus:outline-none bg-gray-200 placeholder-black 
        text-lg sm:text-3xl bg-gradient-to-r from-indigo-100 via-red-400 to-yellow-400`,

  borderBottom: "border-b-4 border-blue-500",

  borderBottomHover:
    "border-b-4 border-blue-500 border-opacity-0 hover:border-opacity-100",
  ...
  headerDropdown: {
    div: (
      ...
      verticalPosition?: string
        ...
        ${horizontalPosition} ${
      verticalPosition ? verticalPosition : "top-6 sm:top-9"
    }
            z-40 bg-blue-400 outline-none shadow-2xl rounded-lg ${
              isVisible ? "" : "hidden"
            }`,
  ...
  inputDropdown: {
    div: (
      isVisible: boolean,
      horizontalPosition: string
    ) => `flex absolute ${horizontalPosition} top-20 
            w-full z-10 outline-none shadow-2xl rounded-lg ${
              isVisible ? "" : "hidden"
            } 
            bg-indigo-200`,
  ...
  imageDisplay:
    "cursor-pointer w-32 h-28 rounded-2xl sm:w-44 sm:h-44 lg:w-48 lg:h-48 xl:w-64 xl:h-64 flex-none",
  ...
  checkout: {
    ...
    header: {
      container:
        "w-full sm:w-11/12 md:w-9/12 flex justify-center border-b cursor-default",
    },

    content: {
      container:
        "w-full sm:w-11/12 md:w-9/12 flex justify-center items-center mb-2 mt-2",
      image: "w-full sm:w-9/12 md:w-8/12 xl:w-5/12",
    },
  },

  productDetailsContainer:
    "flex flex-row flex-initial justify-start capitalize items-center bg-gradient-to-r h-16 mb-12 cursor-default rounded-2xl from-indigo-100 via-red-400 to-yellow-400 w-10/12 sm:w-9/12 md:w-6/12 lg:w-1/3",
};
